...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build one Qdrant vector database (collection) per fake_data category."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("fake_data"),
        help="Path to fake_data directory.",
    )
    parser.add_argument(
        "--collection-prefix",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Remote Qdrant URL (optional). If omitted, local embedded Qdrant is used.",
    )
    parser.add_argument(
        "--qdrant-api-key",
        type=str,
        default=None,
        help="Qdrant API key for remote mode (optional).",
    )
    parser.add_argument(
        "--local-qdrant-path",
        type=Path,
...
def main() -> None:
    ...
    if not records:
        print("No markdown chunks found. Nothing to ingest.")
        return

    by_category: dict[str, list[ChunkRecord]] = {}
    ...
    for category, category_records in sorted(by_category.items()):
        collection_name = normalize_collection_name(
            f"{args.collection_prefix}{category}"
        )

        sample_vector = next(embedder.embed([category_records[0].text]))
        ...
        for batch in batched(category_records, args.batch_size):
        ...
        print(
            f"Collection '{collection_name}' ready: {len(category_records)} chunks from category '{category}'."
        )

    print(f"Done. Total chunks inserted: {total_points}")
